...
static const unsigned char *elf_map(const char *file, size_t *size_out,
...
static const Elf64_Ehdr *elf_header(const unsigned char *map, size_t size,
                                    const char *file) {
...
// Return a pointer to the section header at index i, or NULL on bounds
// violation.
...
static const char *strtab_str(const unsigned char *map, size_t size,
                              const Elf64_Ehdr *eh, uint32_t strtab_idx,
                              uint32_t off) {
...
static bool should_bold(const char *line, size_t len, bool ms_exclusive) {
  ...
  for (int i = 0; BOLD_KEYWORDS[i]; i++) {
    if (contains(line, len, BOLD_KEYWORDS[i]))
      return true;
  }
  return false;
}
...
static int scan_file(const char *file) {
  ...
  bool has_ms_crypto =
      contains(mod_info, mod_info_len, "microsoft_systemcrypto=1");
...

  {
    ...
    while (cursor < mod_end) {
      ...
      size_t line_len = line_end - cursor;
      bool skip = false;
      if (line_len >= 4 && memcmp(cursor, "dep\t", 4) == 0)
        skip = true;
      if (line_len >= 3 && memcmp(cursor, "=>\t", 3) == 0)
        skip = true;
      if (!skip && line_len > 0) {
        print_line(cursor, line_len,
                   should_bold(cursor, line_len, has_ms_crypto));
      }
      cursor = nl ? nl + 1 : mod_end;
    }
  }
  ...
  if (has_ms_crypto) {
    ...
    if (has_syms && has_crypto) {
      ...
      printf("Binary is not using any cryptography, which is FIPS compliant. "
             "(verified symbols table)\n");
    } else {
      printf("Binary does not use a validated cryptographic module. Unknown if "
             "cryptography is in use. (no symbols table)\n");
    }
  }
